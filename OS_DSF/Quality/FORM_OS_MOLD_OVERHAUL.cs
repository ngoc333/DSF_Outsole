...
namespace OS_DSF
{
    public partial class FORM_OS_MOLD_OVERHAUL : Form
    {
        public FORM_OS_MOLD_OVERHAUL()
        {
            
            InitializeComponent();
           // lblTitle2.Text = "Outsole Mold Overhaul by Week ";
        }
        ...
        */
        ...
        private void load_frm()
        {
            try
            {
                ...
                _dt_model = OS_DSF.Addons.Database.LOAD_BY_MODEL(viewMode);
                
               // axfpHeaderData.Hide();
                //axGridModel.Hide();

                dt_top = OS_DSF.Addons.Database.LOAD_BY_TYPE(viewMode);
        ...
        private void FORM_MOLD_REPAIR_WEEKLY_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible)
            {
               // lblPhylon_Click(null, null);
                lblTitle2.Text = "Outsole Mold Overhaul by Week (" + DateTime.Now.AddDays(-7).ToString("MMM-dd") + " ~ " +  DateTime.Now.ToString("MMM-dd") + ")";
                _time_change = 39;
                timer1.Start();
            }
            else
