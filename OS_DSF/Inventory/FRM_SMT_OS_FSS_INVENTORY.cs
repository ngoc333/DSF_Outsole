...
namespace OS_DSF
{
    public partial class FRM_SMT_OS_FSS_INVENTORY : Form
    {
        public FRM_SMT_OS_FSS_INVENTORY()
        {
            InitializeComponent();
        ...
        void DWMYClick(string ButtonCap, string ButtonCD)
        {
            ...
            switch (ButtonCD)
            {
                ...
                case "M":
                    this.Close();
                    Form fc1 = Application.OpenForms["FRM_SMT_OS_FSS_INVENTORY"];
                    if (fc1 != null)
                        fc1.Show();
                    else
                    {
                        FRM_SMT_OS_FSS_INVENTORY f1 = new FRM_SMT_OS_FSS_INVENTORY();
                        f1.Show();
                    }
                    break;
                case "Y":
        ...
        private void BindingData(string arg_op)
        {
            try
            {
                ...
                if (dtsource != null && dtsource.Rows.Count > 0)
                {
                    ...
                    bandOSINV.Caption = Convert.ToDouble(dtsource.Rows[0]["OS_INV"].ToString()).ToString("#,0");
                    bandOSLT.Caption = Convert.ToDouble(dtsource.Rows[0]["OS_LT"].ToString()).ToString("#,0.0");
