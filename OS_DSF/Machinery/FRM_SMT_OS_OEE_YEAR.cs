...
{
    public partial class FRM_SMT_OS_OEE_YEAR : Form
    {
        public FRM_SMT_OS_OEE_YEAR()
        {
            InitializeComponent();
        }
        ...
        private void BindingOEEData(string ARG_QTYPE)
        {
            try
            {
                ...
                DT = SELECT_DATA_OS("GYEAR",uc_year.GetValue().ToString());
               // grdBase.DataSource = DT;

                DataTable dt1 = SELECT_DATA_OS("YEAR", uc_year.GetValue().ToString());
